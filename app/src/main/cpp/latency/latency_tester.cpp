...
private:
    ...
    // 在左声道中寻找高能量的窗口起点，用于语音数字间存在静音的场景
    // 返回样本级的窗口起点集合（已过滤间隔，避免过于密集）
    std::vector<size_t> findHighEnergyWindowStarts(
        ...
        // mean_square > (FS^2 * 10^(-30/10)) = FS^2 * 0.001
        const double fullScaleSq = 32768.0 * 32768.0;
...
//                if (candidates.size() >= 30) break; // 限制候选数量
    ...
    // 检测延迟：使用多窗口归一化互相关方法检测右声道相对左声道的延迟
    // 返回延迟值（毫秒），精确到0.1ms
